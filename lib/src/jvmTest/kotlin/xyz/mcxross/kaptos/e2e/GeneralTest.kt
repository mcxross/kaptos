...
class GeneralTest {
  ...
  fun `it fetches ledger info`() = runBlocking {
    ...
    when (val response = GeneralApi(transportConfig).getLedgerInfo()) {
      is Result.Ok -> {
  ...
  fun `it fetches chain id`() = runBlocking {
    val transportConfig = TransportConfig(AptosSettings(network = Network.LOCAL))
    when (val response = GeneralApi(transportConfig).getChainId()) {
      is Result.Ok -> {
  ...
  fun testGetBlockByBlockHeight() = runBlocking {
    val transportConfig = TransportConfig(AptosSettings(network = Network.LOCAL))
    val height = 1L
    when (val response = GeneralApi(transportConfig).getBlockByHeight(height)) {
      is Result.Ok -> {
  ...
  fun testGetBlockByVersion() = runBlocking {
    val transportConfig = TransportConfig(AptosSettings(network = Network.LOCAL))
    val version = 3L
    when (val response = GeneralApi(transportConfig).getBlockByVersion(version)) {
      is Result.Ok -> {
  ...
  fun `it should fetch chain top user transactions`() = runBlocking {
    ...
    val limit = 10
    when (val response = GeneralApi(transportConfig).getChainTopUserTransactions(limit)) {
      is Result.Ok -> {
        assertEquals(
  ...
  fun testFetchViewFunctionData() = runBlocking {
    ...
    val response =
      GeneralApi(transportConfig)
        .view<List<MoveValue.MoveUint64Type>>(
  ...
  fun testFetchViewFunctionWithBool() {
    runBlocking {
      val transportConfig = TransportConfig(AptosSettings(network = Network.TESTNET))

      val response =
        GeneralApi(transportConfig)
          .view<List<MoveValue.Bool>>(
            InputViewFunctionData(
  ...
  fun testFetchViewFunctionGenericOutput() {
    runBlocking {
      ...
      val resolution =
        GeneralApi(transportConfig)
          .view<List<MoveValue.MoveListType<MoveValue.String>>>(
            InputViewFunctionData(
