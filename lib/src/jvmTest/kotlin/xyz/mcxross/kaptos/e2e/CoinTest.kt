...
class CoinTest {
  ...
  fun `it generates a transfer coin transaction with AptosCoin coin type`() = runBlocking {
    ...
    val payload = assertIs<TransactionPayload.EntryFunction>(rawTransaction.payload)
    val typeArg = assertIs<TypeTagStruct>(payload.call.typeArguments[0])

    expect(true) { typeArg.type.address.toString() == "0x1" }
    expect(true) { typeArg.type.moduleName == "aptos_coin" }
    expect(true) { typeArg.type.name == "AptosCoin" }
  }
  ...
  fun ` it generates a transfer coin transaction with a custom coin type`() = runBlocking {
    ...
    expect(true) { typeArg.type.address.toString() == "0x1" }
    expect(true) { typeArg.type.moduleName == "my_coin" }
    expect(true) { typeArg.type.name == "type" }
  }
