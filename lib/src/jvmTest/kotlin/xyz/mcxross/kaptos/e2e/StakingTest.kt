...
class StakingTest {
  ...
  fun `it queries for the number of delegators`() {
    runBlocking {
      ...
      when (
        val result =
          StakingApi(transportConfig)
            .getNumberOfDelegatorsForAllPools(sortOrder = listOf(sortOrder))
      ) {
        is Result.Ok -> {
          ...
          if (poolAddress != null) {
            ...
            val specificPoolResult =
              StakingApi(transportConfig).getNumberOfDelegators(topPoolAddress)
  ...
  fun `it returns 0 if the poolAddress does not exist`() = runBlocking {
    val transportConfig = TransportConfig(AptosSettings(network = Network.DEVNET))
    val poolAddress =
      "0x12345678901234567850020dfd67646b1e46282999483e7064e70f02f7e12345".toAccountAddress()
    when (val result = StakingApi(transportConfig).getNumberOfDelegators(poolAddress)) {
      is Result.Ok -> {
  ...
  fun `it queries for the activity of a delegator for a given pool`() = runBlocking {
    ...
    when (
      val response =
        StakingApi(transportConfig).getDelegatedStakingActivities(poolAddress, delegatorAddress)
    ) {
      is Result.Ok -> {
