...
class FungibleAssetTest {
  ...
  fun `it should fetch fungible asset metadata`() = runBlocking {
    ...
    val resolution =
      FungibleAssetApi(transportConfig)
  ...
  fun `it should fetch a specific fungible asset metadata by an asset type`() = runBlocking {
    ...
    when (
      val resolution =
        FungibleAssetApi(transportConfig).getFungibleAssetMetadataByAssetType(APTOS_COIN)
    ) {
      is Result.Ok -> {
    ...
    when (
      val resolution =
        FungibleAssetApi(transportConfig)
          .getFungibleAssetMetadataByAssetType("0x1::aptos_coin::testnotexist")
    ) {
      is Result.Ok -> {
  ...
  fun `it should fetch a specific fungible asset metadata by a creator address`() = runBlocking {
    ...
    when (
      val resolution =
        FungibleAssetApi(transportConfig)
          .getFungibleAssetMetadataByCreatorAddress("0xc".toAccountAddress())
    ) {
      is Result.Ok -> {
