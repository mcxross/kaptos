...
class TypeTagTest : StringSpec({
  ...
  "TypeTagStruct toString" {
    TypeTagParser.parseTypeTag("0x1::some_module::SomeResource").toString() shouldBe "0x1::some_module::SomeResource"
  }
  ...
  "struct with nested struct type args" {
    val inner = TypeTagStruct(
      StructTag(AccountAddress.ONE, "coin", "Coin", listOf(TypeTagU64))
    )
    val outer = TypeTagStruct(
      StructTag(AccountAddress.ONE, "wrapper", "Wrap", listOf(inner, TypeTagBool))
    )
