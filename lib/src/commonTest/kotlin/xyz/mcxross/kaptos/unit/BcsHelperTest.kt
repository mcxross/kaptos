...
class BcsHelperTest :
  StringSpec({
    ...
    "MoveVector u8 from ByteArray" {
      Bcs.encodeToByteArray(MoveVector.u8(byteArrayOf(1, 2, 3))).toList() shouldBe
        listOf<Byte>(3, 1, 2, 3)
    }
    ...
    "MoveVector string factory" {
      ...
      encoded.toList() shouldBe
        listOf<Byte>(3, 4, 3, 97, 98, 99, 4, 3, 100, 101, 102, 4, 3, 103, 104, 105)
    }
    ...
    "manual MoveVector MoveString matches factory" {
      val manual =
        Bcs.encodeToByteArray(
          MoveVector(listOf(MoveString("abc"), MoveString("def"), MoveString("ghi")))
        )
    ...
    "rejects MoveOption with length greater than 1" {
      shouldThrow<IllegalArgumentException> {
        Bcs.decodeFromByteArray<MoveOption<U8>>(byteArrayOf(2, 1, 2))
      }
    }
