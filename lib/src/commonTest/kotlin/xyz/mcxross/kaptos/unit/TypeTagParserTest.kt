...
class TypeTagParserTest :
  StringSpec({
    ...
    "parses bool" { TypeTagParser.parseTypeTag("bool") shouldBe TypeTagBool }

    "parses u8" { TypeTagParser.parseTypeTag("u8") shouldBe TypeTagU8 }

    "parses u16" { TypeTagParser.parseTypeTag("u16") shouldBe TypeTagU16 }

    "parses u32" { TypeTagParser.parseTypeTag("u32") shouldBe TypeTagU32 }

    "parses u64" { TypeTagParser.parseTypeTag("u64") shouldBe TypeTagU64 }

    "parses u128" { TypeTagParser.parseTypeTag("u128") shouldBe TypeTagU128 }

    "parses u256" { TypeTagParser.parseTypeTag("u256") shouldBe TypeTagU256 }

    "parses signer" { TypeTagParser.parseTypeTag("signer") shouldBe TypeTagSigner }

    "parses address" { TypeTagParser.parseTypeTag("address") shouldBe TypeTagAddress }
    ...
    "!parses deeply nested struct - BUG: same innerTypes leak issue" {
      val result =
        TypeTagParser.parseTypeTag("$TAG_STRUCT_NAME<$TAG_STRUCT_NAME<$TAG_STRUCT_NAME<u8>>, u8>")
      result.shouldBeInstanceOf<TypeTagStruct>()
    ...
    "parses vector of primitives" {
      val primitives =
        listOf("bool", "u8", "u16", "u32", "u64", "u128", "u256", "address", "signer")
