...
class MoveOptionSerializerTest {
  ...
  fun `unwrap throws when None`() {
    ...
    assertFailsWith<IllegalArgumentException> { decoded.unwrap() }
  }
  ...
  fun `should throw error when deserializing empty byte array`() {
    ...
    assertFailsWith<Exception> { Bcs.decodeFromByteArray<MoveOption<U8>>(emptyInput) }
  }
  ...
  fun `should throw error when deserializing vector with length greater than 1`() {
    ...
    assertFailsWith<IllegalArgumentException> { Bcs.decodeFromByteArray<MoveOption<U8>>(encoded) }
  }
}
