...
internal class DefaultStakingService(private val config: TransportConfig) : StakingService {
  override suspend fun getDelegatorCount(pool: AccountAddressInput): AptosResult<ULong> =
    try {
      when (
        val result =
          getNumberOfDelegatorsData(config, AccountAddress.from(pool), null).toAptosResult()
      ) {
  ...
  override suspend fun getDelegatorCounts(): AptosResult<List<StakingPoolDelegatorCount>> =
    when (val result = getNumberOfDelegatorsForAllPools(config, null).toAptosResult()) {
      is AptosResult.Failure -> result
      is AptosResult.Success ->
        try {
          AptosResult.Success(
          ...
          error.rethrowCancellation()
          AptosResult.Failure(
            AptosError.Serialization("Invalid staking count returned by indexer", error)
          )
        }
    }
        ...
        is AptosResult.Success ->
          AptosResult.Success(
            result.value
              ...
              .orEmpty()
              .map(GetDelegatedStakingActivitiesQuery.Delegated_staking_activity::toRecord)
          )
      }
    } catch (error: Throwable) {
      error.rethrowCancellation()
      AptosResult.Failure(
        AptosError.Serialization("Invalid staking activity returned by indexer", error)
      )
    }
}
